...
math: true
mermaid: true
summary: 'PostgreSQL Lock은 여러 프로세스의 데이터 충돌을 방지하고 일관성을 유지하는 데 사용됨. 이는 행 및 테이블 단위의 다양한 잠금 유형으로 트랜잭션 단위로 작동함. 하지만 과도한 잠금은 성능 저하나 데드락을 유발할 수 있어 관리가 필요하며, `pg_locks` 뷰를 통해 확인하고 최적화하여 문제를 해결함.'
---
