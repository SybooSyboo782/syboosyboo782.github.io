...
math: true
mermaid: true
summary: 'Facade 패턴은 복잡한 하위 시스템에 대한 단순화된 인터페이스를 제공하는 디자인 패턴임. 이를 통해 코드의 책임 분리와 관리 용이성을 높여 개발자가 수정 부분을 명확히 찾을 수 있도록 돕는 장점이 있음. 하지만 Facade 자체가 전지전능한 객체가 되어 모든 클래스에 결합되거나 컴파일 시 의존성을 완전히 제거하기 어려운 한계점도 존재함.'
---
# Facade 패턴
