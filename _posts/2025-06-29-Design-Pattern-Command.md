...
math: true
mermaid: true
summary: '커맨드 패턴은 요청을 객체로 캡슐화하여 요청자(Invoker)와 수행자(Receiver)를 분리하는 디자인 패턴임. 이를 통해 요청 간 결합도를 낮추고, 요청을 큐에 저장하거나 실행 취소(Undo) 기능을 쉽게 구현할 수 있음. 다만, 명령마다 별도 클래스가 필요하여 클래스 수가 많아질 수 있고 단순한 요청에는 복잡도를 증가시킬 수 있음.'
---
# Command(커맨드) 패턴
