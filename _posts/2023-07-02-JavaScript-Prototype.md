...
math: true
mermaid: true
summary: '자바스크립트의 프로토타입은 클래스나 객체 복사 없이 상속을 구현하는 방법임. 객체에 없는 프로퍼티나 메서드에 접근할 때 `__proto__` 연결을 따라 부모 프로토타입 객체를 검색하는 프로토타입 체이닝을 통해 상속이 이루어짐. 이는 자바스크립트에서 상속 관계를 형성하는 핵심 메커니즘임.'
---
