...
math: true
mermaid: true
summary: '도커는 컨테이너 기반의 가상화 플랫폼으로, 소프트웨어와 그 종속성을 독립적인 컨테이너로 패키징하여 격리된 환경에서 실행함. 이를 통해 애플리케이션의 빠르고 일관된 제공 및 유연한 배포를 가능하게 함. 주요 개념으로는 컨테이너를 생성하는 템플릿인 이미지와 이를 관리하는 도커 엔진, 그리고 이미지 저장소인 도커 허브 등이 있음.'
---
